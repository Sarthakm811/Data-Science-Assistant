...
import pandas as pd
import numpy as np
from sklearn.feature_selection import mutual_info_classif, mutual_info_regression, f_classif, chi2
...
class MLReadinessAnalyzer:
    ...
    def __init__(self, df: pd.DataFrame, target_col: Optional[str] = None):
        ...
        if target_col:
            ...
            self.categorical_cols = [c for c in self.categorical_cols if c != target_col]
    
    def assess_ml_readiness(self) -> Dict:
        """Complete ML readiness assessment"""
        results = {
            'overall_score': 0,
            'readiness_grade': 'Not Assessed'
        }
        ...
        if self.target_col and self.target_col in self.df.columns:
            ...
            results['readiness_grade'] = self._get_readiness_grade(results['overall_score'])
        else:
            results.update({
                'transform_recommendations': self.recommend_transforms(),
    ...
    def analyze_imbalance(self) -> Dict:
        ...
        return {
            ...
            'recommendations': self._imbalance_recommendations(imbalance_ratio, minority_count)
        }
    ...
    def _imbalance_recommendations(self, ratio: float, minority_count: int) -> List[str]:
