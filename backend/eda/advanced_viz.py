...
class AdvancedVisualizer:
    ...
    def create_feature_importance(self, ml_data: Dict) -> plt.Figure:
        ...
        colors = [
            "steelblue" if f["type"] == "numeric" else "coral" for f in top_features
        ]
    ...
    def create_pareto_chart(self, col: str) -> plt.Figure:
        ...
        # Bar chart
        ax1.bar(
            range(len(value_counts)), value_counts.values, color="steelblue", alpha=0.7
        )
        ...
        ax2.plot(
            range(len(cumsum)), cumsum.values, color="red", marker="o", linewidth=2
        )
    ...
    def create_quality_overview(self, insights: Dict) -> plt.Figure:
        ...
        bars = ax.barh(
            ...
            scores,
            color=[
                "green" if s >= 80 else "orange" if s >= 60 else "red" for s in scores
            ],
        )
