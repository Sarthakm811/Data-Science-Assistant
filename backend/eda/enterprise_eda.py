...
class EnterpriseEDA:
    ...
    def _generate_executive_summary(self, results: Dict) -> Dict:
        ...
        return {
            ...
            'schema_quality': structural.get('schema_quality', {}).get('quality_score', 0),
            'non_normal_features': statistical.get('summary', {}).get('non_normal_distributions', 0),
    ...
    def _generate_recommendations(self, results: Dict) -> list:
        ...
        if correlations.get('multicollinearity_issues', 0) > 0:
            recommendations.append({
                ...
                'actions': ['Remove or combine highly correlated features', 'Use PCA for dimensionality reduction']
            })
        ...
        if ml_readiness.get('overall_score', 0) < 70:
            recommendations.append({
                ...
                'actions': ['Apply recommended transformations', 'Handle missing data', 'Address imbalance']
            })
        ...
        if leakage.get('leakage_detected'):
            recommendations.append({
                ...
                'actions': [f"Remove feature: {s['feature']}" for s in leakage.get('suspects', [])]
            })
