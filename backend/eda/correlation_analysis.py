...
class CorrelationAnalyzer:
    ...
    def get_correlation_summary(self) -> Dict:
        ...
        return {
            'pearson_high_count': len(all_corr.get('pearson', {}).get('high_correlations', [])),
            'spearman_high_count': len(all_corr.get('spearman', {}).get('high_correlations', [])),
            'categorical_associations': len(all_corr.get('cramers_v', {}).get('high_associations', [])),
            'mixed_correlations': all_corr.get('mixed_correlations', {}).get('count', 0),
            'multicollinearity_issues': len(all_corr.get('vif', {}).get('high_vif_features', [])),
            'redundant_features': all_corr.get('redundancy', {}).get('total_redundant_features', 0)
        }
