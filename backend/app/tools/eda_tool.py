...
class EDATool:
    ...
    def generate_eda_summary(self, results: Dict[str, Any]) -> str:
        ...
        summary.append(f"  - Shape: {info['shape'][0]} rows × {info['shape'][1]} columns")
        ...
        if missing['total_missing'] > 0:
            ...
            for col, data in list(missing['columns_with_missing'].items())[:5]:
                summary.append(f"  - {col}: {data['count']} ({data['percentage']:.1f}%)")
        ...
        if corr.get('high_correlations'):
            ...
            for item in corr['high_correlations'][:5]:
                summary.append(f"  - {item['feature1']} ↔ {item['feature2']}: {item['correlation']:.2f}")
        ...
        if outliers:
            ...
            for col, data in list(outliers.items())[:5]:
                summary.append(f"  - {col}: {data['count']} outliers ({data['percentage']:.1f}%)")
